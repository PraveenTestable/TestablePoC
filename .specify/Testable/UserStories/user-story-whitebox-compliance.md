# Testable Landing Page — Confidence Engine Whitebox Compliance Testing

**Figma File**: Whitebox-Compliance, Whitebox-Compliance_Metric
**Generated**: March 27, 2026

---

## Screen 1: Compliance Testing Dashboard

As a developer or compliance officer reviewing regulatory compliance,
I want to view and interact with compliance testing metrics under the Whitebox Confidence section,
so that I can assess regulatory compliance posture, identify violations, and validate compliance against defined standards.

### 1. Acceptance Criteria

1. The Compliance tab is available under the Gate Analysis section alongside Code Intelligence, Performance, and Security
2. The Compliance tab displays sub-tabs for compliance types: FERPA/COPPA Compliance, GDPR Compliance
3. The active sub-tab is highlighted with distinct background color
4. Each compliance type section displays a header with the compliance type name (e.g., "FERPA/COPPA Compliance Gate") and overall execution status badge
5. Compliance testing categories are grouped under sections (e.g., "Student PII Exposure Scan", "Consent & Age-Gate Compliance")
6. A classification table displays columns: Classification, Value, Execution Status, Result, and Coverage
7. Each classification row displays an icon, name, value with threshold comparison, status badge, result badge, and coverage progress bar
...
16. Sub-tab switching between compliance testing types loads the corresponding classification table without page refresh
17. Each compliance type maintains its own execution state independently
...
20. Value displays with threshold context (e.g., "12/50" showing current value out of threshold)

### 2. Positive Validations

1. Sub-tab switching between compliance testing types loads correct classification data without errors
2. Classification table displays all compliance metrics (PII Data Exposure Detection, COPPA Age Verification Testing, etc.)
...
15. Value displays update correctly when re-running compliance scans
16. Section header status badge updates to "Completed" when all classifications finish
17. Compliance category headers are visually distinct from classification rows
...
15. Compliance scan timeout displays appropriate timeout message with retry option

### 4. Boundary Value Analysis

1. PII exposure count minimum displays as 0 with Pass status
2. PII exposure count at threshold boundary (e.g., exactly 50) displays correct Pass/Warn status
...
7. Very large exposure counts display without breaking layout (e.g., 1000+)
8. Minimum threshold values (e.g., 0) display correctly
9. Maximum threshold values (e.g., 9999+) display without overflow
10. Decimal values display with consistent precision
...
1. All compliance testing sub-tabs maintain consistent height and styling
...
20. Section header includes a download icon for downloading full compliance report
21. Compliance category headers (e.g., Student PII Exposure Scan) are visually distinct from classification rows
22. Value displays show context (e.g., "12/50" with threshold as denominator)

### 6. Accessibility Validations

1. All compliance metrics have appropriate ARIA labels describing the data
...
15. Download button has aria-label describing the action (e.g., "Download PII Data Exposure Detection report")
...
21. Compliance violation counts are announced to screen readers with context
...
2. All compliance testing types display correct data
3. Sub-tab switching between testing types works without errors or data loss
4. Classification tables display all metrics with correct data binding for each compliance type
...
13. Compliance violation indicators display correct colors and announcements

---

## Screen 2: Compliance Metric Detail Modal

As a compliance officer reviewing compliance scan results,
I want to view detailed breakdown of a specific compliance classification metric,
so that I can understand the cumulative metrics and sub-metrics contributing to the compliance assessment.

### 1. Acceptance Criteria

1. The modal displays with header showing the classification name (e.g., "PII Data Exposure Detection")
2. The modal header displays the parent category subtitle (e.g., "Student PII Exposure Scan")
...
6. Value card displays the violation count or metric value prominently
7. Threshold card displays the threshold with comparison operator (e.g., "≥ 70" or "≤ 50")
...
14. A "Re-run" button is available in the modal header to re-execute the compliance scan
...
4. Status badge correctly reflects whether violation count meets threshold
5. Coverage progress bar displays correct percentage and fills proportionally
6. Sub-metrics table displays all child metrics for the compliance classification
...
12. Re-run button triggers re-execution of the compliance scan
...
15. Violation count displays with correct threshold context
...
12. Compliance scan timeout in modal displays appropriate timeout message
...
9. Modal opens correctly for all compliance classification types
10. Modal handles classifications with no sub-metrics gracefully
11. Zero violations displays with Pass status and green indicator
...
17. Value displays in modal match the table view format (e.g., "12/50")
...
17. Violation counts are announced to screen readers with threshold context

### 7. Definition of Done

1. All Positive, Negative, Boundary, UI/UX, and Accessibility validations listed above have been tested and pass across all supported browsers: Chrome, Firefox, Safari, and Edge
2. Modal opens correctly when clicking any compliance classification row
...
11. Re-run functionality executes compliance scan and updates metrics correctly
12. **Download button is NOT present anywhere in the modal popup**
13. Compliance violation indicators display and announce correctly
...
**Note to Developers**: Users must close the modal and use the download button from the table view to download compliance reports. The modal is for viewing detailed compliance metrics only.

---

## Compliance Testing Types Covered

This user story covers the following compliance testing types under the Compliance tab:

1. **FERPA/COPPA Compliance**
   - Student PII Exposure Scan
     - PII Data Exposure Detection
   - Consent & Age-Gate Compliance
     - COPPA Age Verification Testing

2. **GDPR Compliance**
   - Data privacy and protection requirements for EU citizens

---

*Generated from Figma: Whitebox-Compliance, Whitebox-Compliance_Metric*
